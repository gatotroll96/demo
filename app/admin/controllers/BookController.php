<?php
	class BookController extends Controller{
		...
		public function indexAction(){
	    	...
	    	$this->_view->setTitle('Admin Manager: book');
	    ...
	    public function statusAction(){
	    	...
	    	header('location: index.php?module=admin&controller=book&action=index');
	    	exit();
	    }
	    ...
	    public function addAction(){		
	    	...
	    	$this->_templateObj->setFolderTemplate('admin/main/');
	    	$this->_templateObj->load();
	    	$this->_view->setTitle('Admin Manager: ADD book');
	    	...
	    	if(@$this->_arrData['form']['token'] > 0){
	    		...
	    		$validate->addRule('price', 'int', array('min'=> 1, 'max'=>1000));
	    		$validate->addRule('sale_off', 'int1');
	    		$validate->addRule('special', 'status', array('deny'=> array('default')));
	    		...
	    		if($validate->isValid()== false){
	    			...
	    			if($this->_arrData['type'] == 'apply'){
	    				if(isset($this->_arrData['form']['id'])){
	    					...
	    					$link = URL::createLink('admin', 'book', 'add', array('id'=>$lastID));
	    			...
	    			if($this->_arrData['type'] == 'savenew'){
	    				if(isset($this->_arrData['form']['id'])){
	    					$this->_db->editItems($this->_arrData);
	    					header('location: index.php?module=admin&controller=book&action=add');
	    					exit();
	    				}else{
	    					$this->_db->addItems($this->_arrData);
	    					header('location: index.php?module=admin&controller=book&action=add');
